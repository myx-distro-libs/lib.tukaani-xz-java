...
public class IndexDecoder extends IndexBase {
    ...
    public IndexDecoder(final SeekableInputStream in, final StreamFlags streamFooterFlags, final long streamPadding,
	    final int memoryLimit) throws IOException {
	...
	final long endPos = in.position() + streamFooterFlags.backwardSize - 4;
	...
	try {
	    // Number of Records
	    final long count = DecoderUtil.decodeVLI(inChecked);
	    ...
	    // memory usage limit.
	    this.memoryUsage = 1 + (int) ((16L * count + 1023) / 1024);
		...
		assert record == this.recordCount;
    ...
    public boolean hasRecord(final int blockNumber) {
	return blockNumber >= this.recordOffset && blockNumber < this.recordOffset + this.recordCount;
    }

    public boolean hasUncompressedOffset(final long pos) {
	return pos >= this.uncompressedOffset && pos < this.uncompressedOffset + this.uncompressedSum;
    }

    public void locateBlock(final BlockInfo info, long target) {
    ...
    public void setBlockInfo(final BlockInfo info, final int blockNumber) {
	...
	final int pos = blockNumber - this.recordOffset;

	if (pos == 0) {
	    info.compressedOffset = 0;
	    info.uncompressedOffset = 0;
	} else {
	    info.compressedOffset = this.unpadded[pos - 1] + 3 & ~3;
	...
	info.compressedOffset += this.compressedOffset + Util.STREAM_HEADER_SIZE;
	info.uncompressedOffset += this.uncompressedOffset;
    }

    public void setOffsets(final IndexDecoder prev) {
	...
	assert (this.compressedOffset & 3) == 0;
