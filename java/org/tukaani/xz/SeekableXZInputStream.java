...
public class SeekableXZInputStream extends SeekableInputStream {
     ...
     */
    public SeekableXZInputStream(final SeekableInputStream in) throws IOException {
	this(in, -1);
    }
     ...
     *             may be thrown by <code>in</code>
     */
    public SeekableXZInputStream(final SeekableInputStream in, final int memoryLimit) throws IOException {
	this(in, memoryLimit, true);
    }

    /**
     * Creates a new seekable XZ decomporessor with an optional memory usage
     * limit and ability to disable verification of integrity checks.
     * <p>
     ...
     * @since 1.6
     */
    public SeekableXZInputStream(final SeekableInputStream in, int memoryLimit, final boolean verifyCheck)
	    throws IOException {
	...
	{
	    in.seek(0);
	    final byte[] buf = new byte[XZ.HEADER_MAGIC.length];
	...
	while (pos > 0) {
	    if (pos < Util.STREAM_HEADER_SIZE) {
		throw new CorruptedInputException();
	    }

	    // Read the potential Stream Footer.
	    in.seek(pos - Util.STREAM_HEADER_SIZE);
	    inData.readFully(buf);
	    ...
	    pos -= Util.STREAM_HEADER_SIZE;

	    // Decode the Stream Footer and check if Backward Size
	    // looks reasonable.
	    final StreamFlags streamFooter = DecoderUtil.decodeStreamFooter(buf);
		...
		throw new MemoryLimitException(e.getMemoryNeeded() + this.indexMemoryUsage,
			memoryLimit + this.indexMemoryUsage);
	    }
	    ...
	    final long off = index.getStreamSize() - Util.STREAM_HEADER_SIZE;
	    if (pos < off) {
	    ...
	    inData.readFully(buf);
	    final StreamFlags streamHeader = DecoderUtil.decodeStreamHeader(buf);
	...
	for (int i = this.streams.size() - 2; i >= 0; --i) {
	    final IndexDecoder cur = (IndexDecoder) this.streams.get(i);
	    cur.setOffsets(prev);
	    prev = cur;
	}
	...
	final IndexDecoder first = (IndexDecoder) this.streams.get(this.streams.size() - 1);
     ...
     * @since 1.3
     */
    public int getBlockCheckType(final int blockNumber) {
     ...
     * @since 1.3
     */
    public long getBlockCompPos(final int blockNumber) {
	this.locateBlockByNumber(this.queriedBlockInfo, blockNumber);
     ...
     * @since 1.3
     */
    public long getBlockCompSize(final int blockNumber) {
	this.locateBlockByNumber(this.queriedBlockInfo, blockNumber);
	return this.queriedBlockInfo.unpaddedSize + 3 & ~3;
    }
     ...
     * @since 1.3
     */
    public int getBlockNumber(final long pos) {
     ...
     * @since 1.3
     */
    public long getBlockPos(final int blockNumber) {
	this.locateBlockByNumber(this.queriedBlockInfo, blockNumber);
     ...
     * @since 1.3
     */
    public long getBlockSize(final int blockNumber) {
	this.locateBlockByNumber(this.queriedBlockInfo, blockNumber);
	...
	try {
	    ...
	    this.blockDecoder = new BlockInputStream(this.in, this.check, this.verifyCheck, this.memoryLimit,
		    this.curBlockInfo.unpaddedSize, this.curBlockInfo.uncompressedSize);
	} catch (final MemoryLimitException e) {
	    // BlockInputStream doesn't know how much memory we had
	    // already needed so we need to recreate the exception.
	    assert this.memoryLimit >= 0;
	    throw new MemoryLimitException(e.getMemoryNeeded() + this.indexMemoryUsage,
		    this.memoryLimit + this.indexMemoryUsage);
     ...
     * <code>info</code>.
     */
    private void locateBlockByNumber(final BlockInfo info, final int blockNumber) {
	// Validate.
	...
	for (int i = 0;; ++i) {
	    final IndexDecoder index = (IndexDecoder) this.streams.get(i);
     ...
     */
    private void locateBlockByPos(final BlockInfo info, final long pos) {
	...
	if (this.seekPos > this.curPos) {
	    ...
	    final long skipAmount = this.seekPos - this.curPos;
     ...
     * @since 1.3
     */
    public void seekToBlock(final int blockNumber) throws IOException {
	if (this.in == null) {
	    throw new XZIOException("Stream closed");
	}
