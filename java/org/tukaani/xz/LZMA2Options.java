...
public class LZMA2Options extends FilterOptions {
    ...
    private static final int[] presetToDictSize = { 1 << 18, 1 << 20, 1 << 21, 1 << 22, 1 << 22, 1 << 23, 1 << 23,
	    1 << 24, 1 << 25, 1 << 26 };
     ...
     */
    public LZMA2Options(final int dictSize, final int lc, final int lp, final int pb, final int mode, final int niceLen,
	    final int mf, final int depthLimit) throws UnsupportedOptionsException {
	...
	return this.mode == LZMA2Options.MODE_UNCOMPRESSED ? UncompressedLZMA2OutputStream.getMemoryUsage()
	...
	return new LZMA2InputStream(in, this.dictSize, this.presetDict);
    }
	...
	if (dictSize > LZMA2Options.DICT_SIZE_MAX) {
	    throw new UnsupportedOptionsException("LZMA2 dictionary size must not exceed "
		    + (LZMA2Options.DICT_SIZE_MAX >> 20) + " MiB: " + dictSize + " B");
	}
     ...
     */
    public void setLc(final int lc) throws UnsupportedOptionsException {
	...
	if (lc < 0 || lp < 0 || lc > LZMA2Options.LC_LP_MAX || lp > LZMA2Options.LC_LP_MAX
		|| lc + lp > LZMA2Options.LC_LP_MAX) {
	    throw new UnsupportedOptionsException(
		    "lc + lp must not exceed " + LZMA2Options.LC_LP_MAX + ": " + lc + " + " + lp);
	}
     ...
     *             <code>lp</code> exceed LC_LP_MAX
     */
    public void setLp(final int lp) throws UnsupportedOptionsException {
	...
	if (mf != LZMA2Options.MF_HC4 && mf != LZMA2Options.MF_BT4) {
	...
	if (niceLen < LZMA2Options.NICE_LEN_MIN) {
	    throw new UnsupportedOptionsException(
		    "Minimum nice length of matches is " + LZMA2Options.NICE_LEN_MIN + " bytes: " + niceLen);
	}

	if (niceLen > LZMA2Options.NICE_LEN_MAX) {
	    throw new UnsupportedOptionsException(
		    "Maximum nice length of matches is " + LZMA2Options.NICE_LEN_MAX + ": " + niceLen);
	}
	...
	if (pb < 0 || pb > LZMA2Options.PB_MAX) {
	    throw new UnsupportedOptionsException("pb must not exceed " + LZMA2Options.PB_MAX + ": " + pb);
	}
     ...
     *             <code>preset</code> is not supported
     */
    public void setPreset(final int preset) throws UnsupportedOptionsException {
