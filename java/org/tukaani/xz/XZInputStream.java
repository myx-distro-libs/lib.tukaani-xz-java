...
public class XZInputStream extends InputStream {
     ...
     */
    public XZInputStream(final InputStream in) throws IOException {
	this(in, -1);
    }
    ...
    /**
     * Creates a new XZ decompressor with an optional memory usage limit and
     * ability to disable verification of integrity checks.
     * <p>
     ...
     * @since 1.6
     */
    public XZInputStream(final InputStream in, final int memoryLimit, final boolean verifyCheck) throws IOException {
	this.in = in;
	...
	do {
	    ...
	    // of the file.
	    final int ret = inData.read(buf, 0, 1);
	    if (ret == -1) {
	...
	inData.readFully(buf, 4, Util.STREAM_HEADER_SIZE - 4);

	try {
	    this.xzIn = new SingleXZInputStream(this.in, this.memoryLimit, this.verifyCheck, buf);
	...
	try {
		...
		final int ret = this.xzIn.read(buf, off, len);
